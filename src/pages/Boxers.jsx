...
import { assets, boxer } from '../assets/assets'
  ...
  return (
    <div className='mt-10'>
        ...
        <div className='flex justify-between mb-2 md:mb-5'>
            <h1 className='md:text-2xl text-xl font-bold'>Boxers</h1>
            <div className=''>
                <SortBtn variant="red"/>
            </div>
        </div>
        ...
        <div
            ...
            {boxer.map((category, index) => (
            <div key={index}
                className={`hover:-translate-y-[5px] hover:shadow-2xl transition duration-300 rounded ${activeView === 'grid' ? 'md:pt-8 pt-3' : 'flex gap-6 items-center'}`}>
                <Link to={'#'}
                ...
                <div className={activeView === 'list' ? 'pl-6' : 'pl-3'}>
                    ...
                    <p className="text-[#787885]">
                        {category.productDescription.replace(/\//g, ' ')}
                    </p>
                    )}
