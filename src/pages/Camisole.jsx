    ...
    const fetchCamisoles = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          const camisoleProducts = response.data.products.filter(product => {
            if (!product.productName) return false;

            const productName = product.productName.toUpperCase();
            const hasCamisole = productName.includes('CAMISOLE');
            const isWomenCategory = product.category?.name === 'Women';
            const inStock = product.inStock !== false && product.productStock > 0;

            return hasCamisole && isWomenCategory && inStock;
          });

          setFilteredProducts(camisoleProducts);
        }
      } catch (err) {
        console.error('Error fetching camisoles:', err);
        setError('Error loading camisoles. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className='md:mx-10 mx-2 pt-15 md:mt-5 pb-8 px-3 md:px-10'>
        ...
        {loading ? (
          <div className='text-center py-16'>
            ...
            <p className='text-gray-600 mt-4'>Loading camisoles...</p>
          </div>
        ) : error ? (
          <div className='text-center py-16'>
          ...
          <div className='text-center py-16'>
            <p className='text-gray-600 text-lg'>No camisole products found.</p>
