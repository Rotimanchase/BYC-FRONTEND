...
const Pargination = ({ currentPage, totalPages, setCurrentPage }) => {
  ...
  return (
    <div className="flex justify-center items-center gap-2">
      <button type="button" aria-label="Previous" onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1}
      ...
      <div className="flex gap-2 text-gray-500 text-sm md:text-base">
        {[...Array(totalPages).keys()].map((_, index) => (
          <button key={index + 1} type="button" onClick={() => handlePageChange(index + 1)} className={`flex items-center justify-center w-9 md:w-12 h-9 md:h-12 aspect-square border rounded-sm transition-all ${
              currentPage === index + 1 ? 'border-yellow-400 bg-yellow-50' : 'border-gray-300/60 hover:bg-gray-300/10'
            }`}>
            {index + 1}
          </button>
        ))}
      ...
      <button type="button" aria-label="Next" onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages}
