...
const SortBtn = ({ defaultSort = "Most Sold", data, setData, variant = "default" }) => {
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      <button className={`relative w-40 rounded px-3 py-2 font-medium border ${isRed  ? "border-red-500 text-red-600 bg-white hover:bg-red-50" 
            : "border-gray-700 text-gray-800 bg-white hover:bg-gray-50"
        }`} onClick={toggleDropdown}>
        <div className={`absolute -top-2 left-2 bg-white px-1 text-xs uppercase tracking-wide ${isRed ? "text-red-500" : "text-gray-600"}`}>
          Sort By
        </div>
        <div className="flex justify-between items-center w-full">
          <span className="truncate">{selectedSort}</span>
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" className={`ml-2 transform transition-transform ${isOpen ? 'rotate-180' : ''}`} >
      ...
      {isOpen && (
        <ul className="absolute z-10 mt-2 w-40 bg-white border border-gray-200 rounded shadow-lg">
          {sortOptions.map((option) => (
            <li key={option}>
              <button className={`w-full text-left px-4 py-2 text-sm transition-colors ${selectedSort === option ? isRed ? "bg-red-50 text-red-600 font-semibold"
                      : "bg-gray-100 text-gray-900 font-semibold" : "hover:bg-gray-100 text-gray-700"}`} onClick={() => handleSortChange(option)}>
                {option}
              </button>
            </li>
          ))}
